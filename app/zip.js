...
class File {
  ...
  get header() {
    ...
    v.setUint16(6, 8, true); // bit flags
    v.setUint16(8, 0, true); // compression
    v.setUint16(10, 0, true); // modified time
    v.setUint16(12, 0, true); // modified date
    v.setUint32(14, 0, true); // crc32
    v.setUint32(18, 0, true); // compressed size
    v.setUint32(22, 0, true); // uncompressed size
  ...
  directoryRecord(offset) {
    ...
    v.setUint16(4, 0, true); // version made
    v.setUint16(6, 0, true); // version required
    v.setUint16(8, 0, true); // bit flags
    v.setUint16(10, 0, true); // compression
    v.setUint16(12, 0, true); // modified time
    v.setUint16(14, 0, true); // modified date
...
class ZipStreamController {
  ...
  async pull(controller) {
    ...
    console.assert(!this.extra, 'should not have extra here');
...
export default class Zip {
  ...
  get size() {
    const entries = this.files.reduce(
      (total, file) => total + file.byteLength * 2,
      0
    );
    const eod = 22;
    return entries + eod;
